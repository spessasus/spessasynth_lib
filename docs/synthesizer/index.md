...
- startRenderingData - `object`, used for rendering to file. It's formatted as follows:
  ...
  - oneOutput: a `boolean` - indicates the [One output mode](#one-output-mode)
...
- synthConfig → optional, the configuration for audio effects. [See below.](#synth-config)
...
!!! Info
    ...
    The `effectsConfig` in the snapshot within `startRenderingData` override `synthConfig` if provided.

### isReady

A promise that gets resolved when the worklet synthesizer gets fully initialized with the SF3 decoded.
...
```
- config - `ChorusConfig` - the new configuration. Format is described [here.](#chorus-config)
